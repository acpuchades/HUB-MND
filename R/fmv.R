...
fmv_recode_date <- function(data) {
    data %>%
        stringr::str_replace(r"(^(\d{2})/(\d{2})$)", r"(01/\1/\2)") %>%
        stringr::str_replace(r"(^principis? (\d{4})$)", r"(01/01/\1)") %>%
        stringr::str_replace(r"(^mitjans (\d{4})$)", r"(01/06/\1)") %>%
        stringr::str_replace(r"(^(\d{4})$)", r"(01/01/\1)") %>%
...
fmv_data <- readxl::read_excel(fmv_data_path) %>%
    ...
    dplyr::mutate(
        dplyr::across(dplyr::starts_with("fecha_"), fmv_recode_date),
        dplyr::across(dplyr::starts_with("estado_"), fmv_recode_status),
